...
class Flow(object):
    ...
    def __init__(self, flow_el):
        ...
        tx_duration = (
            float(
                int(flow_el.get("timeLastTxPacket")[:-4])
                - int(flow_el.get("timeFirstTxPacket")[:-4])
            )
            * 1e-9
        )
        rx_duration = (
            float(
                int(flow_el.get("timeLastRxPacket")[:-4])
                - int(flow_el.get("timeFirstRxPacket")[:-4])
            )
            * 1e-9
        )
...
# print("End to End Delay: %.4f ms " % ((flow.delayMean * 1e3) / (flow.flowId)))
