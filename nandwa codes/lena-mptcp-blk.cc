...
main (int argc, char *argv[])
{
  ...
  if (useCa)
   {
     ...
     Config::SetDefault ("ns3::LteHelper::NumberOfComponentCarriers", UintegerValue (8));
  ...
  p2ph.SetChannelAttribute ("Delay", TimeValue (NanoSeconds (2.0)));
  ...
  ueLteDevs = wifi.Install (phy, mac, ueNodes);
  ...
  enbLteDevs = wifi.Install (phy, mac, enbNodes);
...
dlClientHelper.SetAttribute ("MaxBytes", UintegerValue (maxBytes));
clientApps.Add (dlClientHelper.Install (remoteHost));
...
p2ph.EnableAsciiAll (ascii.CreateFileStream ("lte1-mptcp-blk.tr"));

AnimationInterface anim ("lte1-mptcp-blk.xml");
anim.SetConstantPosition(ueNodes.Get(0), 0.0, 37.5);
anim.SetConstantPosition((pgw), 75.0, 0.0);
anim.SetConstantPosition(ueNodes.Get(1), 50.0, 37.5);
anim.SetConstantPosition(ueNodes.Get(2), 100.0, 37.5);
...
lteHelper->EnableRlcTraces ();
  ...
  Config::SetDefault ("ns3::ConfigStore::Filename", StringValue ("output-attributeslenamptcp-1048576.txt"));
  ...
  for (uint32_t u = 0; u < ueNodes.GetN (); ++u)
    {
  Ptr<PacketSink> sink1 = DynamicCast<PacketSink> (serverApps.Get (u));
