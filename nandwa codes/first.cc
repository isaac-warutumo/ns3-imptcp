...
main (int argc, char *argv[])
{
  ...
  AnimationInterface anim("first-100packets.xml");
  anim.SetConstantPosition (nodes.Get(0), 10.0, 10.0);
  anim.SetConstantPosition (nodes.Get(1), 30.0, 10.0);
  
  AsciiTraceHelper ascii;
  pointToPoint.EnableAsciiAll(ascii.CreateFileStream("first-100packets.tr"));
 pointToPoint.EnablePcapAll("first-100packets"); 
