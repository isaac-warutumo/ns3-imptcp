...
#include <algorithm>
#include <iostream>
#include <cmath>//required to use power

Cluster::Cluster(int myClusterNodes, int arrayWithNodeCapacities[], int myUpperNodeCapacity) : m_nNodes(myClusterNodes),m_nodeCapacities(arrayWithNodeCapacities), m_upperNodeCapacity(myUpperNodeCapacity) {
...
void Cluster::generateSubsets(int start, int setSize, int myFullSet[], vector< vector<int>>& subsetSumVector2D, vector<int>& subsetSumVector1D)
{
	...
	for (int i = 0; i <static_cast< int>( pow_setSize); i++) {
		vector<int> sumElements;//a vector to hold all the elements that make a sum eg if we get 7 from 3 and 4 then our 
		...
		sumElements.insert(sumElements.begin(), sum);//insert the sum at the beggining of the elements that make up the sum
		sumElements.insert(sumElements.begin() + 1, sumElements.size() - 1);//kep the number of elements in the second element

		//keep the summ elements in the sum vector
...
void Cluster::printHalfSetSumArrays(int myHalfSetSize, vector<vector<int>>& myHalfSetArray2D, vector<int>& myHalfSetArray1D) {
...
int Cluster::findAndPrintSubsets(int myFullSet[], int nElements, int lowerLimit, int upperLimit)
{
	...
	cout << "myFullSet has nElements:" << nElements << " and target lowerLimit: " << lowerLimit << " and upperlimit: " << upperLimit << endl;
	...
	vector< vector<int>> halfSet1Sum2D, halfSet2Sum2D, finalSet2D;//2d vector to store elements forming the sum
	vector<int> halfSet1Sum1D, halfSet2Sum1D, finalSet1D; //1D vector to store sums
	...
	for (int i = 0; i < static_cast<unsigned int>(halfSet1Sum2D.size()); i++) {
		// search for lower bound
		low = lower_bound(halfSet2Sum1D.begin(), halfSet2Sum1D.end(), lowerLimit - halfSet1Sum1D[i]);
		...
		high = upper_bound(halfSet2Sum1D.begin(), halfSet2Sum1D.end(), upperLimit - halfSet1Sum1D[i]);
		...
		for (size_t k = 0; k < nSumElements; k++)
		{
			...
			finalSet1D[1] += halfSet2Sum2D[myLow + k][1];//sum number of elements in halfset1 and halfset2
